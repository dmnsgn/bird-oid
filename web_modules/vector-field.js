import './common/esnext.iterator.map-8a38945d.js';
import './common/function-apply-8ffa8c0a.js';
import './common/object-set-prototype-of-65a42c5e.js';
import './common/redefine-all-0f62e567.js';
 ...
 * @returns {number}
 */
const remap = (n, from1, to1, from2, to2) => from2 + (to2 - from2) * (n - from1) / (to1 - from1);
/**
...
class VectorField {
    ...
    for (let x = 0; x < this.steps[0]; x++) {
      ...
      for (let y = 0; y < this.steps[1]; y++) {
        ...
        for (let z = 0; z < this.steps[2]; z++) {
          ...
          this.field[x][y][z].position = [this.bounds[0] * (x / this.steps[0]), this.bounds[1] * (y / this.steps[1]), this.bounds[2] * (z / this.steps[2])];
          this.field[x][y][z].direction = this.directionFn(this.field[x][y][z].position, [x, y, z]);
        }
      }
    }
  }
  ...
  lookup([cx, cy, cz]) {
    const x = Math.round(remap(cx, -this.halfBounds[0], this.halfBounds[0], 0, this.steps[0]));
    const y = Math.round(remap(cy, -this.halfBounds[1], this.halfBounds[1], 0, this.steps[1]));
    const z = Math.round(remap(cz, -this.halfBounds[2], this.halfBounds[2], 0, this.steps[2]));
